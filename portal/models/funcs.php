...
function deleteUsers($users) {
	global $mysqli; 
	$i = 0;
	$stmt = $mysqli->prepare("DELETE FROM uc_users 
		WHERE id = ?");
	$stmt2 = $mysqli->prepare("DELETE FROM uc_user_permission_matches 
		WHERE user_id = ?");
	foreach($users as $id){
...
function isUserLoggedIn()
{
	...
	$stmt = $mysqli->prepare("SELECT 
		...
		active = 1
		LIMIT 1");
	$stmt->bind_param("is", $user_id, $staff->hash_pw);	
	$stmt->execute();
	$stmt->store_result();
	$num_returns = $stmt->num_rows;
	$stmt->close();
...
function createPermission($permission) {
	global $mysqli; 
	$stmt = $mysqli->prepare("INSERT INTO uc_permissions (
		name
		)
		VALUES (
		?
...
function deletePermission($permission) {
	...
	$i = 0;
	$stmt = $mysqli->prepare("DELETE FROM uc_permissions 
		WHERE id = ?");
	$stmt2 = $mysqli->prepare("DELETE FROM uc_user_permission_matches 
		WHERE permission_id = ?");
	$stmt3 = $mysqli->prepare("DELETE FROM uc_permission_page_matches 
		WHERE permission_id = ?");
	foreach($permission as $id){
		if ($id == 1){
			$errors .= "You cannot delete the default 'New users' group";
		}
		elseif ($id == 2){
			$errors = "You cannot delete the default 'admin' group";
		}
		else{
			$stmt->bind_param("i", $id);
			$stmt->execute();
...
{
	global $mysqli;
	$stmt = $mysqli->prepare("UPDATE uc_permissions
		SET name = ?
		WHERE
		id = ?
		LIMIT 1");
...
function addPermission($permission, $user) {
	global $mysqli; 
	$i = 0;
	$stmt = $mysqli->prepare("INSERT INTO uc_user_permission_matches (
		permission_id,
		user_id
		)
		VALUES (
...
function removePermission($permission, $user) {
	global $mysqli; 
	$i = 0;
	$stmt = $mysqli->prepare("DELETE FROM uc_user_permission_matches 
		WHERE permission_id = ?
		AND user_id =?");
	if (is_array($permission)){
		foreach($permission as $id){
...
{
	global $mysqli;
	$stmt = $mysqli->prepare("UPDATE uc_configuration
		SET 
		value = ?
		WHERE
		id = ?");
...
function createPages($pages) {
	global $mysqli; 
	$stmt = $mysqli->prepare("INSERT INTO uc_pages (
		page
		)
		VALUES (
		?
...
function deletePages($pages) {
	global $mysqli; 
	$stmt = $mysqli->prepare("DELETE FROM uc_pages 
		WHERE id = ?");
	$stmt2 = $mysqli->prepare("DELETE FROM uc_permission_page_matches 
		WHERE page_id = ?");
	foreach($pages as $id){
		$stmt->bind_param("i", $id);
		$stmt->execute();
...
{
	global $mysqli;
	$stmt = $mysqli->prepare("UPDATE uc_pages
		SET 
		private = ?
		WHERE
		id = ?");
...
function addPage($page, $permission) {
	global $mysqli; 
	$i = 0;
	$stmt = $mysqli->prepare("INSERT INTO uc_permission_page_matches (
		permission_id,
		page_id
		)
		VALUES (
...
function fetchPagePermissions($page_id)
{
	...
	$stmt = $mysqli->prepare("SELECT
		id,
		permission_id
		FROM uc_permission_page_matches
		WHERE page_id = ?
		");
...
function removePage($page, $permission) {
	global $mysqli; 
	$i = 0;
	$stmt = $mysqli->prepare("DELETE FROM uc_permission_page_matches 
		WHERE page_id = ?
		AND permission_id =?");
	if (is_array($page)){
		foreach($page as $id){
...
function permission1(){
    ...
    echo "<ul class='nav' id='side-menu'>
        ...
        <li><a href='index.php?page=1.8''><i class='fa fa-files-o fa-fw'></i> Change Password</a></li>
     </ul>";
}
...
function permission2(){
	...
	echo "<ul class='nav' id='side-menu'>
            <li><a href='index.php'><i class='fa fa-dashboard fa-fw'></i> Dashboard</a></li>
            <li><a href='index.php?page=1.8'><i class='fa fa-files-o fa-fw'></i> Change Password</a></li>
            <li><a href='index.php?page=1.4'><i class='glyphicon glyphicon-equalizer'></i> Staff Permissions</a></li>
            <li><a href='index.php?page=1.2'><i class='glyphicon glyphicon-blackboard'></i> System Pages</a></li>
            <li><a href='index.php?page=1.7'><i class='fa fa-th-list'></i> My Staff</a></li>
            <li><a href='index.php?page=1.5'><i class='fa fa-user fa-fw'></i> New Staff</a></li>
            <li><a href='index.php?page=2'><i class='fa fa-th-list'></i> Customers</a></li>
            <li><a href='index.php?page=2.3'><i class='fa fa-user fa-fw'></i> New Customer</a></li>
            <li><a href='index.php?page=3'><i class='fa fa-bar-chart-o'></i> Reports</a></li>
            <li><a href='index.php?page=4'><i class='fa fa-dashboard fa-fw'></i> Logs</a></li>
                <!-- /.nav-second-level -->
            </li>
        </ul>";
